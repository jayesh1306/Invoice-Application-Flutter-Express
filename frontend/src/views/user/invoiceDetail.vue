<template>
  <v-container>
    <v-row justify="center">
      <v-col>
        <v-data-table :headers="headers" :items="invoices" sort-by="calories" class="elevation-1">
          <template v-slot:top>
            <v-toolbar flat>
              ...
              <v-dialog v-model="dialog" max-width="500px">
                <template v-slot:activator="{ on, attrs }">
                  <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on">New Invoice</v-btn>
                </template>
                <v-card>
                  <v-card-title>
...
export default {
  ...
  computed: {
    getInvoices() {
      this.invoices = this.$store.getters.getInvoicesFromStore;
    },
  },
  methods: {
    initialize() {
